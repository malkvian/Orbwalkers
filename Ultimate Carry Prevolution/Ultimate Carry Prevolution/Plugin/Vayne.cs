    ...
    class Vayne : Champion
    {
        ...
        public override void OnCombo()
        {
            ...
            if (Q_Target != null)
            {
                ...
                {
                    if (Bilge.IsReady() && (GetComboDamage(Q_Target) + MyHero.GetAutoAttackDamage(Q_Target) * 6 < Q_Target.Health || GetHealthPercent(Q_Target) < 35))
                        Use_Bilge(Q_Target);

                    if (Botrk.IsReady() && (GetComboDamage(Q_Target) + MyHero.GetAutoAttackDamage(Q_Target) * 6 < Q_Target.Health || GetHealthPercent(Q_Target) < 35))
                        Use_Botrk(Q_Target);
                }
            }
            if (IsSpellActive("R"))
